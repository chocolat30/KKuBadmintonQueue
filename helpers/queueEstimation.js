...
const { getAsync, allAsync } = require("./dbAsync");
 ...
 */
async function calculateAverageDuration(court_id) {
  ...
  return Math.round(totalDuration / validDurations);
}
 ...
 */
async function getQueueWithEstimates(court_id) {
  const [currentMatch, avgDuration, queue] = await Promise.all([
    getAsync("SELECT timestamp FROM current_match WHERE court_id = ? LIMIT 1", [court_id]),
    calculateAverageDuration(court_id),
  ...
  return { queue: queueWithEstimates, avgDuration };
}
